...
class GoldAIEngine:
    ...
    def __init__(self):
        ...
        self.config = load_settings(self.project_root)
        ...
        self.bot = DiscordBot(self.config.get("discord_bot_token", ""), self.config.get("discord_webhook_url", ""))
        self.db = SignalDatabase(os.path.join(data_dir, "signals_history.db"))
        self.optimizer = LearningOptimizer(
            learned_params_path=os.path.join(data_dir, "learned_params.json"),
        )
        self.strategy = HybridStrategy(self.config["params"])
        self.storage = SignalStorage(os.path.join(data_dir, "self_generated_data.db"))
        ...
        # --- Signal Layer Config ---
        self.analysis_timeframes = self.config["analysis_timeframes"]
        self.higher_timeframe_name = self.config.get("higher_timeframe_name", "H4")
        self.setup_confidence_threshold = float(self.config.get("setup_confidence_threshold", 0.62))
        self.confidence_threshold = float(self.config.get("confidence_threshold", 0.75))
        self.trigger_confidence_threshold = float(self.config.get("trigger_confidence_threshold", 0.82))
        self.sure_shot_threshold = float(self.config.get("sure_shot_threshold", 0.88))
        self.point_value = float(self.config.get("point_value", 0.01))
        self.entry_tolerance_points = float(self.config.get("entry_tolerance_points", 120))
        self.signal_cooldown = timedelta(minutes=float(self.config.get("signal_cooldown_minutes", 30)))
        self.max_signal_age_minutes = float(self.config.get("max_signal_age_minutes", 180))
    ...
    def _alert_key(self, decision):
        return f"{decision['mode']}:{decision['direction']}:{decision['timeframe']}"

    def _should_send_alert(self, decision):
    ...
    def _save_timeframe_analysis(self, result, news_alert):
        features = {
    ...
    def _combine_timeframe_results(self, results):
        scores = {"BUY": 0.0, "SELL": 0.0}
        entry_candidates = {"BUY": [], "SELL": []}
        matching_results = {"BUY": [], "SELL": []}
    ...
    def _apply_news_and_double_check(self, combined, high_impact_news):
    ...
    def _build_signal_decision(self, combined):
        direction = combined["direction"]
        confidence = float(combined["confidence"])
        ...
        # Dynamic tolerance based on ATR to define a reasonable "entry window"
        dynamic_tolerance = max(self.entry_tolerance_points * self.point_value, atr * 0.5)

        # Ideal Entry Zone: Tight and precise
        within_ideal_zone = zone_low - (dynamic_tolerance * 0.5) <= current_price <= zone_high + (dynamic_tolerance * 0.5)

        # Momentum Zone: Wider, for high-confidence signals that shouldn't be missed
        ...
        # 1. SURE-SHOT: Ultra high confidence + price is reasonably close (Momentum Zone)
        ...
        # 2. ENTRY_TRIGGER: High confidence + price is in the ideal zone
        if confidence >= self.trigger_confidence_threshold and not combined.get("block_trigger"):
            if within_ideal_zone:
                ...
                # Strong confidence but price is still approaching the zone.
                decision["mode"] = "STRONG_SETUP"
                decision["signal"] = f"STRONG_{direction}"
                return decision

        # 3. SETUP: Minimum confidence threshold met, but not yet a trigger
    ...
    def _news_alert_text(self, high_impact_news):
    ...
    async def run_analysis_cycle(self):
        ...
        # Decide whether to send signal based on the new immediate-trigger logic
        ...
        # Map internal modes to user-friendly Discord labels
        mode_label = {
        ...
        if sent:
            # Save to DB for learning purposes even if it's just a signal
            self.db.save_signal(
                self.config["symbol"],
